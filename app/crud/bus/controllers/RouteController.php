...
use Yii;
use orm\bus\Route;
use orm\bus\search\SearchRoute;
...
class RouteController extends Controller
{
	...
	/**
	 * Tab for viewing SmartRoutes for a single model
	 *
     * @param integer $id
     * @return mixed
	 */
	public function actionViewSmartRoutes($id)
	{
		$model = $this->findModel($id);
		return $this->render('view-relation', [
            'model' => $model,
            'relationName' => 'SmartRoutes',
            'refClass' => 'SmartRoutes',
            'parentColumns' => ['idRoute', 'route'],
            'refColumns' => [
    'idRoute' => 'id',
],
            'dataProvider' => new \yii\data\ActiveDataProvider([
            	'query' => $model->getSmartRoutes(),
            ]),
            'oneToOne' => false,
        ]);
	}
    ...
    {
		$labelFields = explode(",", Route::$__AJAX_LABEL_FIELDS_COMMA_SEPARATED);
		$labelFormat = Route::$__AJAX_LABEL_FORMAT;
