...
if (!isset($formAction)) {
	if ($model->isNewRecord)
		$formAction = ['/event/crud/ticket-price/create'];
	else {
		$formAction = ['/event/crud/ticket-price/update'];
...
?>
	...
	<?php if (in_array('shares', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'shares')->textInput() ?>
	<?php if (in_array('shares', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('currency', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'currency')->dropDownList([
			\orm\event\TicketPrice::CURRENCY_SOLES => 'Soles',
			\orm\event\TicketPrice::CURRENCY_DOLARES => 'Dolares',
		], ['prompt' => '']) ?>
