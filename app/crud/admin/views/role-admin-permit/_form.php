...
if (!isset($formAction)) {
	if ($model->isNewRecord)
		$formAction = ['/admin/crud/role-admin-permit/create'];
	else {
		$formAction = ['/admin/crud/role-admin-permit/update'];
...
?>
	...
	<?php if (in_array('roleAdminId', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'roleAdminId')->textInput() ?>
	<?php if (in_array('roleAdminId', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('priority', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'priority')->textInput()->hint('Number: 1 = Evaluates first, 100 = Evaluates later on') ?>
	<?php if (in_array('priority', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('regex', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'regex')->textInput(['maxlength' => 255])->hint('Regular expression to match route. Eg. demo/(admin|cine)/crud* to match /demo/admin/crud... and /demo/cine/crud...') ?>
	<?php if (in_array('regex', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('type', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'type')->dropDownList([
			\orm\admin\RoleAdminPermit::TYPE_ALLOW => 'Allow',
			\orm\admin\RoleAdminPermit::TYPE_DENY => 'Deny',
		], ['prompt' => ''])->hint('If the regex matches the requested url then we should allow or deny access.') ?>
	<?php if (in_array('type', $hiddenColumns)) echo "</div>"; ?>
