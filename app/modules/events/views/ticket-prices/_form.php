...
if (!isset($formAction)) {
	if ($model->isNewRecord)
		$formAction = ['/event/crud/ticket-prices/create'];
	else {
		$formAction = ['/event/crud/ticket-prices/update'];
...
?>
	...
	<?php if (in_array('priceId', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'priceId')->textInput() ?>
	<?php if (in_array('priceId', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('priceName', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'priceName')->textInput(['maxlength' => 200]) ?>
	<?php if (in_array('priceName', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('shareId', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'shareId')->textInput() ?>
	<?php if (in_array('shareId', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('shareName', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'shareName')->textInput(['maxlength' => 200]) ?>
	<?php if (in_array('shareName', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('currency', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'currency')->dropDownList([
			\orm\event\TicketPrices::CURRENCY_SOLES => 'Soles',
			\orm\event\TicketPrices::CURRENCY_DOLARES => 'Dolares',
		], ['prompt' => '']) ?>
	<?php if (in_array('currency', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('amount', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'amount')->textInput() ?>
	<?php if (in_array('amount', $hiddenColumns)) echo "</div>"; ?>

	<?php if (in_array('paid', $hiddenColumns)) echo "<div style='display:none;'>"; ?>
		<?= $form->field($model, 'paid')->checkbox(null, false) ?>
	<?php if (in_array('paid', $hiddenColumns)) echo "</div>"; ?>
